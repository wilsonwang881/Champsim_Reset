...
private:
  template <typename T>
  ooo_model_instr(T instr, std::array<uint16_t, 2> local_asid) : ip(instr.ip), is_branch(instr.is_branch), branch_taken(instr.branch_taken), asid(local_asid)
  {
    ...
    } else if (!reads_sp && !reads_ip && !reads_flags && writes_ip && reads_other) { // WL: fix from Dr Jimenez.
      // indirect branch
      is_branch = true;
      branch_taken = true;
      branch_type = BRANCH_INDIRECT;
    } else if (!reads_sp && reads_ip && !writes_sp && writes_ip && (reads_flags || reads_other)) { // WL: fix from Dr Jimenez.
      // conditional branch
      is_branch = true;
...
public:
  ooo_model_instr(uint16_t cpu, input_instr instr) : ooo_model_instr(instr, {cpu, cpu}) {}
  ooo_model_instr(uint16_t, cloudsuite_instr instr) : ooo_model_instr(instr, {instr.asid[0], instr.asid[1]}) {}
