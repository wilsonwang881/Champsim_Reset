...
uint64_t spp_l3::prefetcher::issue(CACHE* cache) {
  ...
  if (!ready_queue.empty()) {
    ...
    auto [addr, cycle, priority, RFO_write] = ready_queue.front();
    ...
    if (remaining_acc == -1) {
      std::cout << "Trying to issue " << addr << " for set " << ((addr >> 6) & champsim::bitmask(champsim::lg2(cache->NUM_SET))) << " at cycle " << cache->current_cycle << " MSHR usage: " << mshr_occupancy << " queue size " << ready_queue.size() << " wq " << wq_occupancy << " rq " << rq_occupancy << std::endl;
    ...
    if (!RFO_write && mshr_occupancy < cache->get_mshr_size()) { 
      if (way == cache->NUM_WAY && search_mshr == cache->MSHR.end()) {
        ...
        if (prefetched) {
          ...
          if (ready_queue.size() % 100000 == 0) 
            ready_queue.shrink_to_fit();

          issued_pf.insert((addr >> 6) << 6);
          total_issued_pf++;

          if (debug_print) 
            std::cout << "Issued " << addr << " for set " << ((addr >> 6) & champsim::bitmask(champsim::lg2(cache->NUM_SET))) << " at cycle " << cache->current_cycle << " MSHR usage: " << mshr_occupancy << " queue size " << ready_queue.size() << " wq " << wq_occupancy << " rq " << rq_occupancy << std::endl;
        }
      }
      else {
        ready_queue.pop_front();
      ...
      if (debug_print) 
        std::cout << "Issue WRITE operation " << addr << " for set " << ((addr >> 6) & champsim::bitmask(champsim::lg2(cache->NUM_SET))) << " at cycle " << cache->current_cycle << " MSHR usage: " << mshr_occupancy << " queue size " << ready_queue.size() << " wq " << wq_occupancy << " rq " << rq_occupancy << std::endl;
    }
  }
...
bool spp_l3::prefetcher::call_poll() {
  ...
  if (std::get<0>(potential_cs_pf) != 0) {
    ...
    auto pq_insert_it = std::find_if(ready_queue.begin(), ready_queue.end(), pq_place_at);
    ready_queue.emplace(pq_insert_it,std::get<0>(potential_cs_pf), std::get<1>(potential_cs_pf), std::get<2>(potential_cs_pf), std::get<3>(potential_cs_pf));
