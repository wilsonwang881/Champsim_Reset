...
void spp::SPP_PAGE_BITMAP::init()
{
  for(size_t i = 0; i < TABLE_SIZE; i++)
  {
    ...
    for (size_t j = 0; j < BITMAP_SIZE; j++) 
    {
      ...
      tb[i].bitmap_delta[j] = false;
    }

    for (size_t k = 0; k <DELTA_SIZE; j++) 
    {
      tb[i].delta[k] = 0;
      tb[i].c_delta[k] = 0;
      tb[i].lru_delta[k] = 0;
  ...
  for(size_t i = 0; i < TABLE_SIZE; i++)
  {
    ...
    for (size_t j = 0; j < BITMAP_SIZE; j++) 
    {
      dr[i].bitmap_delta[j] = false;
    }

    for (size_t j = 0; j <DELTA_SIZE; j++) 
    {
      dr[i].delta[j] = false;
      dr[i].c_delta[j] = false;
      dr[i].lru_delta[j] = false;
    }
  }
  */
}
...
void spp::SPP_PAGE_BITMAP::update(uint64_t addr)
{
  ...
  //HL
  for(auto &var : tb[index].bitmap_delta)
    var = false;
...
void spp::SPP_PAGE_BITMAP::update_bitmap_store()
{
  ...
  //clear the state for every round,clear the bitmap and the last offset, retain delta table
  for (size_t i=0;i<TABLE_SIZE;i++)
  {
    tb[i].last_offset=0;
    for (size_t j = 0; j < BITMAP_SIZE; j++)
    {
      tb[i].bitmap_delta[j]=0;
    }
  }
}
  ...
  for(auto var : i_lru_vec) 
  {
    ...
    if (tb[i].page_no == tb[i].page_no_store) 
    {
      ...
      for (size_t j = 0; j < BITMAP_SIZE; j++) 
      {
        ...
        if(tb[i].bitmap_delta[j] && !(tb[i].bitmap[j] && tb[i].bitmap_store[j]))
        {
            cs_pf.push_back(std::make_pair(page_addr + (j << 6), false)); 
        }
      } 
