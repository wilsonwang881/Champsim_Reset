...
void spp_l3::SPP_ORACLE::file_read() {
  ...
  for (int set_partition = 0; set_partition < MEMORY_USAGE_REDUCTION_FACTOR; set_partition++) {
    ...
    std::map<uint64_t, std::deque<uint64_t>> addr_t_map;
    ...
    for (int i = readin.size() - 1; i >= 0; i--) {
      ...
      if (tmpp_readin.miss_or_hit == 1) {
        ...
        readin[i].t = addr_t_map[tmpp_readin.addr];
        addr_counter_map.erase(tmpp_readin.addr);
        addr_timestamp_map.erase(tmpp_readin.addr);
        addr_t_map.erase(tmpp_readin.addr);
      }
    }

    for(auto var : readin) {
      if (var.miss_or_hit != 0) {
        assert(var.t.size() == (size_t)var.miss_or_hit);
...
int spp_l3::SPP_ORACLE::update_pf_avail(uint64_t addr, uint64_t cycle) {
  ...
  for (i = set * WAY_NUM; i < (set + 1) * WAY_NUM; i++) {
    ...
    if (cache_state[i].addr == addr) {
      ...
      cache_state[i].t.pop_front();
      assert(cache_state[i].t.size() == (size_t)cache_state[i].pending_accesses);
    ...
    while (set_availability[set] > 0 && oracle_pf[set].size() > 0) {
      ...
      if (way < WAY_NUM) {
        ...
        */
        ...
        cache_state[set * WAY_NUM + way].t = ite->t;
...
uint64_t spp_l3::SPP_ORACLE::rollback_prefetch(uint64_t addr) {
  ...
  */
  ...
  latest_cycle = cache_state[set * WAY_NUM].t.front();

  // Use the timestamp for LRU replacement.
  // Evict block that has not been accessed for long time.
  for (uint64_t i = set * WAY_NUM; i < (set + 1) * WAY_NUM; i++) {
    if (cache_state[i].t.front() < latest_cycle) {
      index = i;
      latest_cycle = cache_state[i].t.front();
    }
  }
