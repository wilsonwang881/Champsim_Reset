...
void page_bitmap::prefetcher::gather_pf()
{
  ...
  for(size_t i = 0; i < TAG_COUNTER_SIZE; i++) 
  {
    if (upt[i].valid)
    {
      std::cout << std::setw(4) << (unsigned)i << std::setw(3) << (unsigned)upt[i].counter << "|"; 
    }  
...
void page_bitmap::prefetcher::perceptron_update(uint64_t addr, bool useful)
{
  ppf.tag_counter_update(addr, useful);
}

void page_bitmap::prefetcher::invalidate_p_tb(bool rj_or_pf_tb, uint64_t addr)
{
  if (rj_or_pf_tb)
    ppf.invalidate_rj_pf_tb(ppf.rj_tb, addr); 
  else
    ppf.invalidate_rj_pf_tb(ppf.pf_tb, addr);
}

void page_bitmap::prefetcher::update_p_tb(bool rj_or_pf_tb, uint64_t addr)
{
  if (rj_or_pf_tb)
    ppf.update_rj_pf_tb(ppf.rj_tb, addr); 
  else 
    ppf.update_rj_pf_tb(ppf.pf_tb, addr);
}
...
bool page_bitmap::prefetcher::check_p_tb(bool rj_or_pf_tb, uint64_t addr)
{
  ...
  if (rj_or_pf_tb)
    res = ppf.check_rj_pf_tb(ppf.rj_tb, addr); 
  else
